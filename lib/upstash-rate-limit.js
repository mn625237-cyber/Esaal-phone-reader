...
function getClientIp(req) {
  // Vercel provides x-real-ip as the normalized client address. Prefer it over
  // x-forwarded-for because forwarded headers can contain a client-supplied
  // value or a changing proxy chain and would create a new Redis key per request.
...
async function checkRateLimit({ req, endpoint, limit }) {
  ...
  try {
    ...
    console.info('[rate-limit] Upstash counter result', {
      endpoint,
